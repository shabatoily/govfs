...
- 서버가 정한 최대 입력 크기를 초과하면 업로드 전에 거부한다.
...
현재 VFS 변경 API는 작업을 비동기로 실행하고 `202 Accepted`를 반환한다. 유효한 `X-Client-ID`가 있으면 성공 또는 실패 결과를 해당 SSE 구독자에게 알리며, 없으면 작업만 실행한다. MCP 클라이언트도 SSE를 구독한 뒤 발급받은 client ID로 완료 이벤트를 기다린다. 동일 리소스에 대한 요청을 동시에 구분해야 할 필요가 생기면 응답과 이벤트에 공통 작업 ID를 추가한다.
